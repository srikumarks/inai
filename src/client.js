...
function stdHeaders() {
    ...
    if (document.body.hasAttribute("token")) {
        headers.append(
            "Authorization",
            "Bearer " + document.body.getAttribute("token")
        );
    }
    return headers;
}
...
async function boot() {
    ...
    if (document.body.hasAttribute("inai-boot")) {
        ...
        headlessServices.push.apply(
            headlessServices,
            document.body.getAttribute("inai-boot").trim().split(/\s+/)
        );
    }
...
function serviceArgs(inai_name) {
    let inai_args = null;
    try {
        let argsScript = document.querySelector(
            'script[inai-args-for="' + inai_name + '"]'
        );
        ...
        if (argsScript) {
            ...
            inai_args = JSON.parse(
                decodeURIComponent(element.getAttribute("inai-args"))
            );
        }
    } catch (e) {}
    return inai_args;
...
function setupElementService(element, codeCache) {
    if (!(element instanceof Element) || !element.hasAttribute("inai")) {
        return;
    }
    let codeId = element.getAttribute("inai");
    let inai_name = element.hasAttribute("id")
    ...
    if (!codeP) {
        ...
        codeCache[codeId] = codeP = (async () => {
            ...
            let response = await fetch(
                providerURLBase + "/_codebase/" + codeId,
                {
                    method: "GET",
                    headers: stdHeaders(),
