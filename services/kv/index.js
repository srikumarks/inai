...
async function boot(args) {
    ...
    let write_redis = I.require("redis").createClient({
        host: write_host,
        port: write_port,
    });
    ...
    let branch = (headers) => {
        return (headers && headers["inai-branch"]) || null;
    };
    ...
    I.get = async function (name, resid, query, headers) {
        try {
            if (query && query.prefix) {
                return {
                    status: 200,
                    body: await getPrefix(name, resid, query, headers),
                };
            }
            if (query && query.list) {
                return {
            ...
            if (br) {
                ...
                return {
                    status: 200,
                    body: JSON.parse(cpstr === null ? str : cpstr),
                };
            } else {
    ...
    I.put = async function (name, resid, query, headers, body) {
        try {
            ...
            if (query && query.list) {
                await wdbcall(query.unique ? "sadd" : "rpush", [
                    dbkey(branch(headers), key),
                    ...body,
                ]);
            } else {
                await wdbcall("set", [
    ...
    I.delete = async function (name, resid, query, headers, body) {
        ...
        await I.atomic(async () => {
            ...
            if (keys && result !== keys.length) {
                throw new Error(
                    "Found " + keys.length + " keys but deleted only " + result
                );
            }
        });
        ...
        await I.atomic(async () => {
            ...
            if (keys && keys.length > 0) {
                ...
                for (let i = 0; i < keys.length; ++i) {
                    txn.rename(
                        keys[i],
                        keys[i].replace("/b/" + brid + "/" + brns, "/" + brns)
                    );
                }
                await txnExec(txn);
            }
    ...
    async function getPrefix(name, resid, query, headers) {
        if (/[*]/.test(resid)) {
            return {
                status: 400,
                body: "Can't use wild card in prefix search.",
            };
        }
    ...
    async function getList(getter, name, resid, query, headers) {
        ...
        let members = await dbcall(
            getter,
            getter === "lrange" ? [dbkey(null, key), 0, -1] : [dbkey(null, key)]
        );
        if (!members) {
            throw "Now found";
        }
