I.boot = async function mainBoot(name, resid, query, headers, config) {
    ...
    I.post = async function (name, resid, query, headers, body) {
                ...
                if (isForProfile(spec, req.auth.profile)) {
                    ...
                    if (spec.config) {
                        res.set(
                            ...
                            encodeURIComponent(
                                JSON.stringify(
                                    resolveEnvVar(spec.config, I.env)
                                )
                            )
                        );
                    }
        ...
        router.put(
            /[/]_codebase[/]code[/]([^/]+)$/,
            onlyLocalhost(async function (req, res) {
                await I.network(
                    "_codebase",
                    "put",
                    "/code/" + req.params[0],
        ...
        let proxyHandler = withAuth(async function (req, res) {
            ...
            if (!perms) {
                perms =
                    (req.auth.services && req.auth.services[json.name]) || null; // It has permissions to access the service.
            }
            if (!perms) {
                let spec = (
                    await I.network(
        ...
        function installPublicHandler() {
            router.use(async function (req, res, next) {
                ...
                if (
                    spec &&
                    (!spec.public ||
                        (spec.env && spec.env.indexOf("server") < 0))
                ) {
                    res.status(404).send("Not found");
        ...
        function start(mountPoint) {
            servers[serverRefID] = new Promise((resolve, reject) => {
                ...
                setTimeout(function () {
                    if (!started) {
                        console.error(
                            "Server not started for 30 seconds. Aborting!"
                        );
