...
function validBranchID(secret, branch) {
    ...
    return hmac(secret, parts[0]).substring(0, 8) === parts[1]
        ? parts[0]
        : null;
}
...
I.boot = async function (name, resid, query, headers, config) {
    ...
    const allowedDomainsPat = new RegExp(
    ...
    const kTokenCookieRE = new RegExp(
        "[;]" + kInaiAuthCookieName + "=([^;]+)[;]"
    );
    ...
    async function getKnownUser(id) {
        ...
        let groups = await I.network(
            userdb,
            "get",
            "/auth/users/" + id + "/groups",
            null,
            null
        );
        user.groups = new Set();
            ...
            if (groups.body) {
                for (let g of groups.body) {
                    ...
                    if (gs.status === 200) {
                        ...
                        console.assert(
                            user.groups.has(g),
                            "A super group should also be in the groups list."
                        );
                    } else {
                        user.groups.add(g);
                    }
    ...
    function unpackAuthToken(authorization) {
        ...
        let context = branch
            ...
            : "";
        let calcSig = hmac(
            knownApps[kSystemId].secret,
            context + appId + "." + salt + "." + time
        );
        return {
            branch: branch,
            user: userid,
    ...
    function validatedTokenInfo(info) {
        ...
        if (info.sig !== info.calcSig) {
            return { status: "invalid" };
        }
        if (Date.now() >= info.time + I.token_expiry_ms) {
            return { status: "expired", info: info };
        }
    ...
    function useCookies(headers) {
        if (
            headers &&
            headers["set-cookie"] &&
            typeof headers["set-cookie"] === "object"
        ) {
            let kvpairs = [];
            ...
            headers.cookie =
                (headers.cookie && headers.cookie.trim().length > 0
                    ? headers.cookie + "; "
                    : "") + kvpairsLine;
        }
    }
        ...
        try {
            switch (resid) {
                case "/check": {
                    ...
                    if (!givenAuthorization) {
                        break;
                    }
                    let c_auth = authCache.get(givenAuthorization);
                    if (
                        c_auth &&
                        c_auth.ts > I.earliest_renew_time_ms &&
                        Date.now() < c_auth.ts + I.token_expiry_ms
                    ) {
                        return { status: 200, body: c_auth.auth };
                    }
                    let { status, info } = validatedTokenInfo(
                        unpackAuthToken(givenAuthorization)
                    );
                    if (status === "expired") {
                        let renew = await I.post(
                            name,
                            "/token",
                            info,
                            null,
                    ...
                    authCache.set(givenAuthorization, {
                        ts: tokenInfo.time,
                        auth: auth,
                    });
                ...
                case "/token": {
                    if (
                        ...
                        if (
                            !info ||
                            info.sig !== info.calcSig ||
                            info.time < I.earliest_renew_time_ms
                        ) {
                    ...
                    if (!knownApps[query.app]) {
                        throw "forbidden";
                    }
                    let now = Date.now();
                    if (now >= query.time + I.token_expiry_ms) {
                        throw "token_expired";
                    }
                    let mysig = hmac(
                        knownApps[kSystemId].secret,
                        query.salt + "." + query.time
                    );
                    let clisig = hmac(
                        knownApps[query.app].secret,
                        query.app +
                            "." +
                            query.salt +
                            "." +
                ...
                case "/users": {
                    ...
                    if (
                        !user ||
                        !user.email ||
                        !allowedDomainsPat.test(user.email)
                    ) {
                        throw "forbidden";
                    }
                    ...
                    let userid = hmac(
                        knownApps[kSystemId].secret,
                        JSON.stringify([user.iss, user.sub, user.email])
                    );
                    let { status, info } = validatedTokenInfo(
                        unpackAuthToken(givenAuthorization)
                    );
            ...
            if (pat) {
                ...
                if (gs.status === 200) {
                    ...
                    if (body.length === 0) {
                        return {
                            status: 400,
                            body: "No valid group names given",
                        };
                    }
