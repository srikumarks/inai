...
I.boot = async function boot(name, resid, query, headers, config) {
    ...
    let tableArgPat = new RegExp('([\\s]+)(from|update|(insert\\s+into))[\\s]+(([A-Za-z0-9_]+)|([\'][^\']+[\'])|([\`][^\`]+[\`]))', 'gi');

    I.post = async function (name, resid, query, headers, sql) {
        let res = resid.match(/^[/]?([A-Za-z0-9_]+)[/]_query[/]?$/);
        if (!res) { return { status: 404 }; }

        let auth = await I.network('auth', 'post', '/check', null, headers, null);
        ...
        try {
            ...
            let results = await sequelize.query(sqlt, { replacements: { table_arg: res[1], raw: true } });
